...
// Widget body
$title_link_text = esc_html__('View All','hot-coffee');
if(isset($args['taxonomy']) && 'product_cat' == $args['taxonomy']){
	$title_link_text = esc_html__('Shop Now','hot-coffee');
}
				...
				?><div class="<?php echo esc_attr(trx_addons_get_column_class(1, $columns, $columns_tablet, $columns_mobile)); ?>"><?php
					...
					<div class="categories_list_item tilt_action_cat_list">
						...
						$src = empty($image)
							...
							: trx_addons_add_thumb_size($image, hot_coffee_get_thumb_size($columns > 2 ? 'big' : 'huge'));
						$attr = trx_addons_getimagesize($src);
						?><img src="<?php echo esc_url($src); ?>" <?php if (!empty($attr[3])) trx_addons_show_layout($attr[3]); ?> alt="<?php esc_attr_e('Category image', 'hot-coffee'); ?>">
						</div><?php
						?>
						<div class="categories_list_info_wrap">
							<div class="categories_list_info">
								...
								<a href="<?php echo esc_url($cat_link); ?>" class="categories_link_more"><?php hot_coffee_show_layout($title_link_text); ?></a>
							</div>
						</div>
