...
if ( apply_filters( 'hot_coffee_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$hot_coffee_with_featured_image = hot_coffee_is_with_featured_image( $hot_coffee_post_header, array( 'with_gallery' ) );
	// Post title and meta
	ob_start();
	...
	if ( strpos( $hot_coffee_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $hot_coffee_with_featured_image ) {
                echo ' with_featured_image' . ( false && hot_coffee_get_theme_option( 'single_parallax' ) == 0 ? ' hot-coffee-full-height' : '' );
            }
		?>">
			<?php
