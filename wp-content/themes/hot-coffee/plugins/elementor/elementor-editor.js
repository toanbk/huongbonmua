...
jQuery( document ).ready(
	function() {
		...
		hot_coffee_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="hot_coffee_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.hot_coffee_post_editor' );
						if ( link.length > 0 ) {
							hot_coffee_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.hot_coffee_post_editor', function(e) {
						hot_coffee_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('hot_coffee_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function hot_coffee_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
