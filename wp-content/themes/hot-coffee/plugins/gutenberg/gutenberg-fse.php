...
if ( ! defined( 'HOT_COFFEE_FSE_TEMPLATE_PART_PT' ) ) define( 'HOT_COFFEE_FSE_TEMPLATE_PART_PT', 'wp_template_part' );

if ( ! defined( 'HOT_COFFEE_FSE_TEMPLATE_PART_AREA_HEADER' ) )  define( 'HOT_COFFEE_FSE_TEMPLATE_PART_AREA_HEADER',  defined( 'WP_TEMPLATE_PART_AREA_HEADER' )  ? WP_TEMPLATE_PART_AREA_HEADER  : 'header' );
if ( ! defined( 'HOT_COFFEE_FSE_TEMPLATE_PART_AREA_FOOTER' ) )  define( 'HOT_COFFEE_FSE_TEMPLATE_PART_AREA_FOOTER',  defined( 'WP_TEMPLATE_PART_AREA_FOOTER' )  ? WP_TEMPLATE_PART_AREA_FOOTER  : 'footer' );
if ( ! defined( 'HOT_COFFEE_FSE_TEMPLATE_PART_AREA_SIDEBAR' ) ) define( 'HOT_COFFEE_FSE_TEMPLATE_PART_AREA_SIDEBAR', defined( 'WP_TEMPLATE_PART_AREA_SIDEBAR' ) ? WP_TEMPLATE_PART_AREA_SIDEBAR : 'sidebar' );
if ( ! defined( 'HOT_COFFEE_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) define( 'HOT_COFFEE_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED', defined( 'WP_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ? WP_TEMPLATE_PART_AREA_UNCATEGORIZED : 'uncategorized' );

// Load additional files with FSE support
require_once hot_coffee_get_file_dir( 'plugins/gutenberg/gutenberg-fse-lists.php' );
require_once hot_coffee_get_file_dir( 'plugins/gutenberg/gutenberg-fse-templates.php' );


if ( ! function_exists( 'hot_coffee_gutenberg_fse_loader' ) ) {
	add_action( 'wp_loaded', 'hot_coffee_gutenberg_fse_loader', 1 );
	/**
	 * Turn off a Gutenberg (plugin) templates on frontend
	 * if a theme-specific frontend editor is active.
	 */
	function hot_coffee_gutenberg_fse_loader() {
		// 
		if ( hot_coffee_exists_gutenberg_fse()
			&& get_option( 'show_on_front' ) == 'page'
			&& (int) get_option('page_on_front') > 0
			&& hot_coffee_is_on( hot_coffee_get_theme_option( 'front_page_enabled', false ) )
			&& hot_coffee_get_current_url() == '/'
		) {
...
if ( ! function_exists( 'hot_coffee_gutenberg_is_fse_wp_support' ) ) {
	/**
	 * Check if WordPress with FSE support installed
	 * (WordPress 5.9+ Site Editor is present).
	 */
	function hot_coffee_gutenberg_is_fse_wp_support() {
		return function_exists( 'wp_is_block_theme' );
	}
}

if ( ! function_exists( 'hot_coffee_gutenberg_is_fse_gb_support' ) ) {
	/**
	 * Check if a plugin "Gutenberg" with FSE support is installed.
	 */
	function hot_coffee_gutenberg_is_fse_gb_support() {
		return function_exists( 'gutenberg_is_fse_theme' );
	}
}

if ( ! function_exists( 'hot_coffee_exists_gutenberg_fse' ) ) {
	/**
	 * Check if FSE is available
	 * (WordPress 5.9+ or the plugin "Gutenberg" are installed).
	 */
	function hot_coffee_exists_gutenberg_fse() {
		return hot_coffee_exists_gutenberg() && ( hot_coffee_gutenberg_is_fse_wp_support() || hot_coffee_gutenberg_is_fse_gb_support() );
	}
}

if ( ! function_exists( 'hot_coffee_gutenberg_is_fse_theme' ) ) {
	/**
	 * Check if the current theme supports a FSE behaviour.
	 */
	function hot_coffee_gutenberg_is_fse_theme() {
		static $fse = -1;
		if ( $fse == -1 ) {
			if ( hot_coffee_gutenberg_is_fse_wp_support() ) {                                    // WordPress 5.9+ FSE
				$fse = (bool) wp_is_block_theme();
			} else if ( hot_coffee_gutenberg_is_fse_gb_support() ) {                             // Plugin Gutenberg FSE
				$fse = (bool) gutenberg_is_fse_theme();
			} else {
				$fse = is_readable( HOT_COFFEE_THEME_DIR . 'templates/index.html' )              // WordPress 5.9+ FSE
						|| ( HOT_COFFEE_THEME_DIR != HOT_COFFEE_CHILD_DIR && is_readable( HOT_COFFEE_CHILD_DIR . 'templates/index.html' ) )
						|| is_readable( HOT_COFFEE_THEME_DIR . 'block-templates/index.html' )    // Plugin Gutenberg FSE
						|| ( HOT_COFFEE_THEME_DIR != HOT_COFFEE_CHILD_DIR && is_readable( HOT_COFFEE_CHILD_DIR . 'block-templates/index.html' ) );
			}
		}
		return $fse;
	}
...
if ( ! function_exists( 'hot_coffee_gutenberg_is_fse_enabled' ) ) {
	/**
	 * Check if the current theme supports FSE behaviour and FSE is installed and activated
	 */
	function hot_coffee_gutenberg_is_fse_enabled() {
		return hot_coffee_exists_gutenberg_fse() && hot_coffee_gutenberg_is_fse_theme();
	}
}

if ( ! function_exists( 'hot_coffee_gutenberg_fse_skin_updated' ) ) {
	add_action( 'hot_coffee_action_theme_updated', 'hot_coffee_gutenberg_fse_skin_updated' );
	add_action( 'hot_coffee_action_skin_updated', 'hot_coffee_gutenberg_fse_skin_updated', 10, 1 );
	/**
	 * Copy FSE-required folders from an updated skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'hot_coffee_action_skin_updated', 'hot_coffee_gutenberg_fse_skin_updated', 10, 1 );
	 */
	function hot_coffee_gutenberg_fse_skin_updated( $skin = '' ) {
		// Get an active skin name if empty
		if ( current_action() == 'hot_coffee_action_theme_updated' && empty( $skin ) ) {
			$skin = hot_coffee_skins_get_active_skin_name();
		}
		if ( hot_coffee_skins_get_current_skin_name() == $skin ) {
			hot_coffee_gutenberg_fse_copy_from_skin( $skin );
			hot_coffee_gutenberg_fse_update_theme_json( $skin );
		}
	}
}

if ( ! function_exists( 'hot_coffee_gutenberg_fse_copy_from_skin' ) ) {
	add_action( 'hot_coffee_action_skin_switched', 'hot_coffee_gutenberg_fse_copy_from_skin', 10, 2 );
	/**
	 * Copy FSE-required folders from a new skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'hot_coffee_action_skin_switched', 'hot_coffee_gutenberg_fse_copy_from_skin', 10, 2 );
	 */
	function hot_coffee_gutenberg_fse_copy_from_skin( $new_skin, $old_skin = '' ) {
		$theme_templates_dir = hot_coffee_prepare_path( HOT_COFFEE_THEME_DIR . ( hot_coffee_gutenberg_is_fse_wp_support() ? 'templates/' : 'block-templates/' ) );
		$theme_template_parts_dir = hot_coffee_prepare_path( HOT_COFFEE_THEME_DIR . ( hot_coffee_gutenberg_is_fse_wp_support() ? 'parts/' : 'block-template-parts/' ) );
		$theme_json = hot_coffee_prepare_path( HOT_COFFEE_THEME_DIR . 'theme.json' );
		$skin_templates_dir = hot_coffee_prepare_path( HOT_COFFEE_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( hot_coffee_gutenberg_is_fse_wp_support() ? '/templates/' : '/block-templates/' ) );
		$skin_template_parts_dir = hot_coffee_prepare_path( HOT_COFFEE_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( hot_coffee_gutenberg_is_fse_wp_support() ? '/parts/' : '/block-template-parts/' ) );
		// Remove old templates from the stylesheet dir (if exists)
		if ( is_dir( $theme_templates_dir ) ) {
			hot_coffee_unlink( $theme_templates_dir );
		}
		if ( is_dir( $theme_template_parts_dir ) ) {
			hot_coffee_unlink( $theme_template_parts_dir );
		}
		if ( file_exists( $theme_json ) ) {
			hot_coffee_unlink( $theme_json );
		}
		...
		if ( is_dir( $skin_templates_dir ) ) {
			hot_coffee_copy( $skin_templates_dir, $theme_templates_dir );
		}
		if ( is_dir( $skin_template_parts_dir ) ) {
			hot_coffee_copy( $skin_template_parts_dir, $theme_template_parts_dir );
		}
	}
}

if ( ! function_exists( 'hot_coffee_gutenberg_fse_update_theme_json' ) ) {
	add_action( 'hot_coffee_action_skin_switched', 'hot_coffee_gutenberg_fse_update_theme_json', 30, 2 );
	add_action( 'hot_coffee_action_save_options', 'hot_coffee_gutenberg_fse_update_theme_json', 30 );
	add_action( 'trx_addons_action_save_options', 'hot_coffee_gutenberg_fse_update_theme_json', 30 );
	/**
	 * Update the file 'theme.json' after the current skin is switched and/or theme options are saved.
	 * 
	 * Hooks: add_action( 'hot_coffee_action_skin_switched', 'hot_coffee_gutenberg_fse_update_theme_json', 30, 2 );
	 * 
	 *        add_action( 'hot_coffee_action_save_options', 'hot_coffee_gutenberg_fse_update_theme_json', 30 );
	 * 
	 *        add_action( 'trx_addons_action_save_options', 'hot_coffee_gutenberg_fse_update_theme_json', 30 );
	 * 
	 * Trigger filter 'hot_coffee_filter_gutenberg_fse_theme_json_data' to allow other modules modify this data before saving.
	 */
	function hot_coffee_gutenberg_fse_update_theme_json( $new_skin = '', $old_skin = '' ) {
		// Get an active skin name if empty
		if ( in_array( current_action(), array( 'hot_coffee_action_save_options', 'trx_addons_action_save_options' ) ) && empty( $new_skin ) ) {
...
if ( ! function_exists( 'hot_coffee_gutenberg_fse_theme_json_data' ) ) {
	/**
	 * Return a default data to save to the file 'theme.json'
	 * 
	 * Trigger filter 'hot_coffee_filter_gutenberg_fse_theme_json_data' to allow other modules change data.
	 * 
	 * @return array  Data for the 'theme.json'
	 */
	function hot_coffee_gutenberg_fse_theme_json_data() {
...
*/
		...
		if ( ! empty( $scheme[ hot_coffee_get_scheme_color_name( 'bg_color' ) ] ) && ! empty( $scheme[ hot_coffee_get_scheme_color_name( 'text' ) ] ) ) {
			$data['settings']['color']['duotone'][] = array(
				"colors" => array( $scheme[ hot_coffee_get_scheme_color_name( 'bg_color' ) ], $scheme[ hot_coffee_get_scheme_color_name( 'text' ) ] ),
				"slug"   => 'bg-and-text',
				"name"   => esc_html__( 'Background and text color', 'hot-coffee' )
			);
		}
		if ( ! empty( $scheme[ hot_coffee_get_scheme_color_name( 'bg_color' ) ] ) && ! empty( $scheme[ hot_coffee_get_scheme_color_name( 'text_dark' ) ] ) ) {
			$data['settings']['color']['duotone'][] = array(
				"colors" => array( $scheme[ hot_coffee_get_scheme_color_name( 'bg_color' ) ], $scheme[ hot_coffee_get_scheme_color_name( 'text_dark' ) ] ),
				"slug"   => 'bg-and-dark',
				"name"   => esc_html__( 'Background and dark color', 'hot-coffee' )
			);
		}
		if ( ! empty( $scheme[ hot_coffee_get_scheme_color_name( 'bg_color' ) ] ) && ! empty( $scheme[ hot_coffee_get_scheme_color_name( 'text_link' ) ] ) ) {
			$data['settings']['color']['duotone'][] = array(
				"colors" => array( $scheme[ hot_coffee_get_scheme_color_name( 'bg_color' ) ], $scheme[ hot_coffee_get_scheme_color_name( 'text_link' ) ] ),
				"slug"   => 'bg-and-link',
				"name"   => esc_html__( 'Background and link color', 'hot-coffee' )
			);
		}

		// Add gradients
		if ( ! empty( $scheme[ hot_coffee_get_scheme_color_name( 'text_hover' ) ] ) && ! empty( $scheme[ hot_coffee_get_scheme_color_name( 'text_link' ) ] ) ) {
			$data['settings']['color']['gradients'][] = array(
				...
				"name"     => esc_html__( 'Vertical from link color to hover color', 'hot-coffee' ),
			);
			$data['settings']['color']['gradients'][] = array(
