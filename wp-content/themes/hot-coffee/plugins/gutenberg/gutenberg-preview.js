...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			hot_coffee_create_observer( 'hot-coffee-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				hot_coffee_gutenberg_editor_init();
			} );
		} else {
		 	hot_coffee_gutenberg_editor_init();
		}
	...
	function hot_coffee_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'hot_coffee_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'hot_coffee_inited' ) ) {
	...
	// Init on page load
	function hot_coffee_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = hot_coffee_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + HOT_COFFEE_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + HOT_COFFEE_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in HOT_COFFEE_STORAGE['override_classes'] ) {
				$obj.addClass( HOT_COFFEE_STORAGE['override_classes'][i].replace( '%s', HOT_COFFEE_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( hot_coffee_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && HOT_COFFEE_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'hot_coffee_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			hot_coffee_remove_observer( 'hot-coffee-check-editor-styles-wrapper' );
			hot_coffee_create_observer( 'hot-coffee-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'hot_coffee_inited' );
	}
} );
