...
if ( ! function_exists( 'hot_coffee_storage_get' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_get( $var_name, $default = '' ) {
		global $HOT_COFFEE_STORAGE;
		return isset( $HOT_COFFEE_STORAGE[ $var_name ] ) ? $HOT_COFFEE_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'hot_coffee_storage_set' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_set( $var_name, $value ) {
		global $HOT_COFFEE_STORAGE;
		$HOT_COFFEE_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'hot_coffee_storage_empty' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $HOT_COFFEE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $HOT_COFFEE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $HOT_COFFEE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'hot_coffee_storage_isset' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $HOT_COFFEE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $HOT_COFFEE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $HOT_COFFEE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'hot_coffee_storage_unset' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $HOT_COFFEE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $HOT_COFFEE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $HOT_COFFEE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'hot_coffee_storage_inc' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_inc( $var_name, $value = 1 ) {
		global $HOT_COFFEE_STORAGE;
		if ( empty( $HOT_COFFEE_STORAGE[ $var_name ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ] = 0;
		}
		$HOT_COFFEE_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'hot_coffee_storage_concat' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_concat( $var_name, $value ) {
		global $HOT_COFFEE_STORAGE;
		if ( empty( $HOT_COFFEE_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'hot_coffee_storage_get_array' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $HOT_COFFEE_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] ) ? $HOT_COFFEE_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $HOT_COFFEE_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $HOT_COFFEE_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'hot_coffee_storage_set_array' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_set_array( $var_name, $key, $value ) {
		global $HOT_COFFEE_STORAGE;
		if ( ! isset( $HOT_COFFEE_STORAGE[ $var_name ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$HOT_COFFEE_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'hot_coffee_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function hot_coffee_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $HOT_COFFEE_STORAGE;
		if ( ! isset( $HOT_COFFEE_STORAGE[ $var_name ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$HOT_COFFEE_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$HOT_COFFEE_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'hot_coffee_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_merge_array( $var_name, $key, $value ) {
		global $HOT_COFFEE_STORAGE;
		if ( ! isset( $HOT_COFFEE_STORAGE[ $var_name ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$HOT_COFFEE_STORAGE[ $var_name ] = array_merge( $HOT_COFFEE_STORAGE[ $var_name ], $value );
		} else {
			$HOT_COFFEE_STORAGE[ $var_name ][ $key ] = array_merge( $HOT_COFFEE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'hot_coffee_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $HOT_COFFEE_STORAGE;
		if ( ! isset( $HOT_COFFEE_STORAGE[ $var_name ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			hot_coffee_array_insert_after( $HOT_COFFEE_STORAGE[ $var_name ], $after, $key );
		} else {
			hot_coffee_array_insert_after( $HOT_COFFEE_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'hot_coffee_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function hot_coffee_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $HOT_COFFEE_STORAGE;
		if ( ! isset( $HOT_COFFEE_STORAGE[ $var_name ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			hot_coffee_array_insert_before( $HOT_COFFEE_STORAGE[ $var_name ], $before, $key );
		} else {
			hot_coffee_array_insert_before( $HOT_COFFEE_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'hot_coffee_storage_push_array' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_push_array( $var_name, $key, $value ) {
		global $HOT_COFFEE_STORAGE;
		if ( ! isset( $HOT_COFFEE_STORAGE[ $var_name ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $HOT_COFFEE_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] ) ) {
				$HOT_COFFEE_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $HOT_COFFEE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'hot_coffee_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $HOT_COFFEE_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $HOT_COFFEE_STORAGE[ $var_name ] ) && is_array( $HOT_COFFEE_STORAGE[ $var_name ] ) && count( $HOT_COFFEE_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $HOT_COFFEE_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] ) && is_array( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] ) && count( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'hot_coffee_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function hot_coffee_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $HOT_COFFEE_STORAGE;
		if ( ! isset( $HOT_COFFEE_STORAGE[ $var_name ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ][ $key ] = 0;
		}
		$HOT_COFFEE_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'hot_coffee_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function hot_coffee_storage_concat_array( $var_name, $key, $value ) {
		global $HOT_COFFEE_STORAGE;
		if ( ! isset( $HOT_COFFEE_STORAGE[ $var_name ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $HOT_COFFEE_STORAGE[ $var_name ][ $key ] ) ) {
			$HOT_COFFEE_STORAGE[ $var_name ][ $key ] = '';
		}
		$HOT_COFFEE_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'hot_coffee_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $HOT_COFFEE_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $HOT_COFFEE_STORAGE[ $var_name ] ) ? $HOT_COFFEE_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $HOT_COFFEE_STORAGE[ $var_name ] ) ? $HOT_COFFEE_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'hot_coffee_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function hot_coffee_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $HOT_COFFEE_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $HOT_COFFEE_STORAGE[ $var_name ]->$prop ) ? $HOT_COFFEE_STORAGE[ $var_name ]->$prop : $default;
	}
}
