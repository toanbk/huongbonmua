...
if ( function_exists( 'hot_coffee_elementor_is_preview' ) && hot_coffee_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			hot_coffee_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	hot_coffee_new_main_query(
		array(
			'post_type'      => hot_coffee_get_theme_option( 'post_type' ),
			'category'       => hot_coffee_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'hot_coffee_filter_get_template_part', hot_coffee_blog_archive_get_template() ) );
}
